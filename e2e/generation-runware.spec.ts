/**
 * E2E Test: Generation with Runware Provider
 * 
 * 强制走 Runware 供应商的完整流程测试
 * 
 * 验证：
 * - gen_start → gen_ok → results_ok 事件链
 * - gen_route 事件（provider = runware）
 * - 1024 预览
 * - 水印
 * - 无 EXIF
 * - 下载解锁（已付）
...
test.describe("E2E Test: Generation with Runware Provider", () => {
  test.describe.configure({ retries: 1, timeout: 90_000 }) // 90 秒超时

  test.beforeEach(async ({ page, request }) => {
    await dismissNextOverlay(page)

    // Set provider weights to force Runware
  ...
  test("完整流程：强制 Runware → 生成 → 预览 → 付款 → 下载", async ({
    page,
    request,
  }) => {
    ...
    await test.step("3️⃣ 生成（强制 Runware）", async () => {
      console.log("\n=== 3️⃣ 生成（强制 Runware）===")
    ...
    // ===== 4️⃣ 验证 gen_route 事件（provider = runware）=====
    await test.step("4️⃣ 验证 gen_route 事件（provider = runware）", async () => {
      console.log("\n=== 4️⃣ 验证 gen_route 事件（provider = runware）===")
