...
**目前實作狀態**：所有 API 端點都支援 **Mock 模式**，當 `NEXT_PUBLIC_USE_MOCK=true` 時：

- ✅ **不會實際呼叫外部圖片生成服務**（如 Runware、FAL）
- ✅ **使用內存狀態機模擬生成流程**（90 秒完成）
- ✅ **返回 Mock 圖片 URL**（使用本地資源或占位符服務）
- ✅ **不需要 API Key**，適合開發與測試環境

**未來擴充**：當需要接真實 provider 時，會在 `lib/generation/provider-router.ts` 中擴充，不影響現有 Mock 流程。
