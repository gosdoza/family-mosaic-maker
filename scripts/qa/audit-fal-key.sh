...
# 
# 目标：
# 1. 盘点本机、Vercel（Preview/Prod）是否存在 FAL_API_KEY
# 2. 扫描代码：USE_MOCK、NEXT_PUBLIC_USE_MOCK、fal-client.ts、/api/generate 是否有「无 key → 自动降级/Mock」分支
# 3. 打 /api/health 与 /api/generate：确认 FAL 子检查是否被 skip、生成是否走 Mock
# 4. 产出 /docs/qa/fal_key_audit.md 报告
...
REPORT_FILE="${REPORT_FILE:-fal_key_audit.md}"
...
if [ -f "$LOCAL_ENV_FILE" ]; then
  echo "  检查本地 .env.local..."
  LOCAL_FAL_KEY=$(grep -E "^FAL_API_KEY=" "$LOCAL_ENV_FILE" | cut -d'=' -f2- | tr -d '"' || echo "")
...
# 生成环境矩阵
LOCAL_FAL_STATUS=$([ -n "$LOCAL_FAL_KEY" ] && echo "✅ 存在" || echo "❌ 缺失")
...
|------|-------------|----------|---------------------|
| **本地 (.env.local)** | ${LOCAL_FAL_STATUS} | ${LOCAL_USE_MOCK_STATUS} | ${LOCAL_NEXT_PUBLIC_USE_MOCK_STATUS} |
...
# 检查 fal-client.ts
FAL_CLIENT_FILE="lib/generation/fal-client.ts"
FAL_CLIENT_FALLBACK=""
if [ -f "$FAL_CLIENT_FILE" ]; then
  echo "  扫描 $FAL_CLIENT_FILE..."
  if grep -q "FAL_API_KEY" "$FAL_CLIENT_FILE"; then
    if grep -q "mock\|fallback\|degradation" "$FAL_CLIENT_FILE" -i; then
      FAL_CLIENT_FALLBACK="✅ 存在 fallback/Mock 分支"
    else
      FAL_CLIENT_FALLBACK="⚠️  未找到明确的 fallback 逻辑"
    fi
  else
    FAL_CLIENT_FALLBACK="❌ 未检查 FAL_API_KEY"
  fi
else
  FAL_CLIENT_FALLBACK="❌ 文件不存在"
fi
...
if [ -f "$GENERATE_ROUTE_FILE" ]; then
  ...
  if grep -q "USE_MOCK\|NEXT_PUBLIC_USE_MOCK" "$GENERATE_ROUTE_FILE"; then
    if grep -q "FAL_API_KEY" "$GENERATE_ROUTE_FILE"; then
      if grep -q "mock\|fallback" "$GENERATE_ROUTE_FILE" -i; then
        GENERATE_ROUTE_MOCK="✅ 存在 Mock 分支和 FAL_API_KEY 检查"
      else
        GENERATE_ROUTE_MOCK="⚠️  存在 USE_MOCK 和 FAL_API_KEY 检查但未找到明确的 Mock 逻辑"
      fi
    else
      GENERATE_ROUTE_MOCK="⚠️  存在 USE_MOCK 但未检查 FAL_API_KEY"
    fi
  else
...
if [ -f "$HEALTH_ROUTE_FILE" ]; then
  ...
  if grep -q "fal\|Fal\|FAL" "$HEALTH_ROUTE_FILE" -i; then
    if grep -q "skip\|error\|missing\|fail-fast" "$HEALTH_ROUTE_FILE" -i; then
      HEALTH_FAL_CHECK="✅ 存在 skip/error/fail-fast 处理"
    else
      HEALTH_FAL_CHECK="⚠️  检查 fal 但未找到明确的错误处理"
    fi
  else
    HEALTH_FAL_CHECK="❌ 未检查 fal"
  fi
else
  HEALTH_FAL_CHECK="❌ 文件不存在"
fi
...
HEALTH_FAL_STATUS=$(echo "$HEALTH_RESPONSE" | jq -r '.fal.ok // "unknown"' 2>/dev/null || echo "unknown")
HEALTH_FAL_ERROR=$(echo "$HEALTH_RESPONSE" | jq -r '.fal.error // "none"' 2>/dev/null || echo "none")

echo "    fal.ok: ${HEALTH_FAL_STATUS}"
echo "    fal.error: ${HEALTH_FAL_ERROR}"
...
# 准备变量值
LOCAL_FAL_STATUS_TEXT=$([ -n "$LOCAL_FAL_KEY" ] && echo "存在" || echo "缺失")
HEALTH_STATUS_TEXT=$([ "$HEALTH_FAL_STATUS" = "false" ] && echo "✅ 正确显示错误状态" || [ "$HEALTH_FAL_STATUS" = "true" ] && echo "⚠️  显示 OK，可能未检查 key" || echo "⚠️  状态未知")
HEALTH_ERROR_TEXT=$([ "$HEALTH_FAL_ERROR" != "none" ] && echo "✅ 有错误信息" || echo "⚠️  无错误信息")
...
1. **环境状态**: 本地环境 REPORT_LOCAL_FAL_STATUS_PLACEHOLDER FAL_API_KEY；Vercel 环境需手动验证。
...
| 文件 | 检查项 | 结果 |
|------|--------|------|
| **lib/generation/fal-client.ts** | FAL_API_KEY 检查 + fallback | REPORT_FAL_CLIENT_FALLBACK_PLACEHOLDER |
| **app/api/generate/route.ts** | USE_MOCK 分支 + Mock 逻辑 | REPORT_GENERATE_ROUTE_MOCK_PLACEHOLDER |
| **app/api/health/route.ts** | fal 子检查 + 错误处理 | REPORT_HEALTH_FAL_CHECK_PLACEHOLDER |
...
**请求**:
```bash
curl -s REPORT_PRODUCTION_URL_PLACEHOLDER/api/health | jq '.fal'
```

**响应**:
```json
{
  "ok": REPORT_HEALTH_FAL_STATUS_PLACEHOLDER,
  "error": "REPORT_HEALTH_FAL_ERROR_PLACEHOLDER"
}
```
...
- **v1.0.0** (REPORT_DATE_PLACEHOLDER): 初始版本，完成 FAL API Key 缺失全检报告
REPORT_EOF
...
replacements = {
    ...
    "REPORT_LOCAL_FAL_STATUS_PLACEHOLDER": "$LOCAL_FAL_STATUS_TEXT",
    "REPORT_HEALTH_FAL_STATUS_PLACEHOLDER": "$HEALTH_FAL_STATUS",
    "REPORT_HEALTH_FAL_ERROR_PLACEHOLDER": "$HEALTH_FAL_ERROR",
    "REPORT_PRODUCTION_URL_PLACEHOLDER": "$PRODUCTION_URL",
    "REPORT_FAL_CLIENT_FALLBACK_PLACEHOLDER": "$FAL_CLIENT_FALLBACK",
    "REPORT_GENERATE_ROUTE_MOCK_PLACEHOLDER": "$GENERATE_ROUTE_MOCK",
    "REPORT_HEALTH_FAL_CHECK_PLACEHOLDER": "$HEALTH_FAL_CHECK",
...
PYTHON_SCRIPT
  ...
  sed -i '' "s/REPORT_LOCAL_FAL_STATUS_PLACEHOLDER/${LOCAL_FAL_STATUS_TEXT}/g" "$REPORT_DIR/$REPORT_FILE"
  sed -i '' "s/REPORT_HEALTH_FAL_STATUS_PLACEHOLDER/${HEALTH_FAL_STATUS}/g" "$REPORT_DIR/$REPORT_FILE"
  sed -i '' "s/REPORT_HEALTH_FAL_ERROR_PLACEHOLDER/${HEALTH_FAL_ERROR}/g" "$REPORT_DIR/$REPORT_FILE"
  sed -i '' "s|REPORT_PRODUCTION_URL_PLACEHOLDER|${PRODUCTION_URL}|g" "$REPORT_DIR/$REPORT_FILE"
  sed -i '' "s|REPORT_FAL_CLIENT_FALLBACK_PLACEHOLDER|${FAL_CLIENT_FALLBACK}|g" "$REPORT_DIR/$REPORT_FILE"
  sed -i '' "s|REPORT_GENERATE_ROUTE_MOCK_PLACEHOLDER|${GENERATE_ROUTE_MOCK}|g" "$REPORT_DIR/$REPORT_FILE"
  sed -i '' "s|REPORT_HEALTH_FAL_CHECK_PLACEHOLDER|${HEALTH_FAL_CHECK}|g" "$REPORT_DIR/$REPORT_FILE"
...
echo "3. 验证修补项："
echo "   - Fail-Fast Gate: curl -s ${PRODUCTION_URL}/api/health | jq '.fal'"
echo "   - CI Gate: NODE_ENV=production NEXT_PUBLIC_USE_MOCK=false FAL_API_KEY= pnpm predeploy:guard"
